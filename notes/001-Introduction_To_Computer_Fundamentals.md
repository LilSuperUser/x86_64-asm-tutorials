...
- **Control Bus** -> Carries signals related to control operations (example: read or write signals)
...
## 3. Memory Hierarchy
The **memory hierarchy** refers to the different level of memory storage in a computer, each with varying speeds and capacity to store data.
...
### 2. **Cache Memory**
   - **Function**:
      - Stores frequently accessed data and instructions to reduce time to access data from RAM.
...
#### 4.1.2 **Addressing Modes**
  - The techniques used to specify operands (data) for instructions. Common addressing modes include:
    ...
    - **Indirect Addressing**: The address of the operand is specified by a register or memory location.
...
#### 4.2.4 **EPIC (Explicitly Parallel Instruction Computing)**
  - Designed to exploit instruction-level parallelism by explicitly specifying parallel operation in the instruction set.
...
When an interrupt occurs, the CPU performs the following steps:
  ...
  2. **Interrupt service routine (ISR)**:
     - The CPU jumps to a special function called an `interrupt service routine (ISR)` or `interrupt handler`, which is responsible for handling the interrupt.
  3. **Handle the event**:
...
When an exception occurs, the following steps are typically performed:
